...
\citep{Calvin01} projiziert 2D und 3D Gehirnbilder aus MRTs auf Kopfphantom für Gehirnoperationen.
...
Bilderinformationen werden bei ersterem in einer einzigen Daten mit der Endung .nii gespeichert. Um mit dem Analyze-Format kompatibel zu bleiben besitzen auch NIfTI-Dateien einen 348 Byte Header. 
...
Im Folgenden werden grundlegende Techniken des Volume Rendering erläutert.
...
Die Klassifikation von Voxeln bestimmt deren optische Eigenschaften in Abhängigkeit zu ihrem Isowert. In diesem Schritt wird somit definiert, welche Farbe und Opazität die einzelnen Voxel, wodurch verschiedene Bereichen, wie z.B. Gewebestrukturen voneinander unterscheidbar werden. Die Qualität des gerenderten Bildes hängt zu einem großen Teil von der Klassifikation ab.Oft wird eine Transferfunktion verwendet, um Voxel zu klassifizieren. Auf diese wird im folgenden Abschnitt näher eingegangen. 

Das Ergebnis einer Klassifikation hängt von dem Zeitpunkt ab, zu dem diese durchgeführt wird. Sie kann durchlaufen werden vor oder nach der Interpolation der Volumendaten.

...
Die Daten müssen interpoliert werden, da das Volumen wie mehrfach beschrieben in einzelnen Werten vorliegt. Der Raum zwischen diesen Werten ist nicht definiert. Um ein zusammenhängendes Volumen darstellen zu können muss also eine Interpolation zwischen den Werten stattfinden. Diese wird in der Regel durch die Verwendung eines Filters bzw. einer Convolution umgesetzt. Sie findet während des Rasterisierungsschrittes statt?
...
Falls eine Transferfunktion eine hohe Frquenz, schnelle Veränderung auffweist, können diese bei der Abtastung des Volumens durch Strahlen verlohren gehen, wenn die Abstände zwischen den Abtastungspunkten zu groß sind. 
Dies ist z.B. der Fall bei Pre-Klassifikation. 
Um dem entgegen zu wirken, kann die Abtastungsfrequenz des Strahl in der Post-Klassifikation erhöht werden. Dies resultiert allerdings in einen höheren Rechenaufwand, der das Rendering verlangsamt. 
Um dieses Problem zu lösen kann die pre-integrierte Klassifikation angewandt werden. Dabei ist der abstand zwischen den Abtastungspunkten konstant. Es werden jeweils zwei Abtastungspunkten betrachtet, wobei einer der vordere und einer der hintere ist. 
Um den Alphawert eines Punktes $i$ zu bestimmen, wird ein Integral über die Alphawerte von $i$ und $i+1$ berechenet. Der Integral beschreibt die Fläche des unter der Transferfunktion liegenden Bereichs zwischen zwei Punkten. 
Der negative Wert des Integrals wird als Exponent einer Exponentialfunktion eingesetzt und der daraus resultierende Wert schließlich von $1$ abgezogen. Das Endergebnis ist der Opazitätswert des Punktes $i$.

Segmente statt voxel
Funktion für alpha (K), funktion für lookup iso (s), funktion um uv werte zu erhalten (x)
lambda ist p, das etlang strahl erhöh wird
Die Berechnung des Alphawertes $\alpha_i$ des $i$-ten Segments wird wie folgt berechnet:

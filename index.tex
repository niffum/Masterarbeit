\documentclass[a4paper,11pt]{report}
\usepackage{geometry}
\geometry{verbose,a4paper,tmargin=25mm,bmargin=25mm,lmargin=25mm,rmargin=25mm}
\usepackage[utf8]{inputenc}
\usepackage[singlespacing]{setspace}
\usepackage{textcomp}
\usepackage{blindtext}
\usepackage{titlesec}
\setlength{\marginparwidth}{2cm}
\usepackage{todonotes}
\usepackage[ngerman]{babel}
\usepackage{listingsutf8}
\usepackage{minted}
\usepackage{parskip}
\usepackage{verbatim}
\usepackage{graphicx}
\usepackage{chngcntr}
\usepackage{csquotes}
\usepackage[citestyle=authoryear,natbib=true,backend=bibtex]{biblatex}
\usepackage{footnote}
\usepackage[section]{placeins}
\usepackage{algpseudocode}
\usepackage{algorithm}
\usepackage{array}
\usepackage{threeparttable}
\usepackage{pgfplots}
\usepackage{longtable}
\usepackage[automake,acronym,xindy,toc]{glossaries}
\usepackage{hyperref}
\usepackage[ngerman]{cleveref}
\usepackage{makeidx}   % load package
\usepackage{amsmath}
\usepackage{booktabs}
\usepackage{longtable}
\usepackage{caption}
\captionsetup{justification=raggedright,singlelinecheck=false}

\setcounter{biburllcpenalty}{7000}
\setcounter{biburlucpenalty}{8000}

\newcommand*{\bildquelle}[1]{\par\raggedleft\footnotesize Quelle:~#1}

\let\oldcite\cite
\renewcommand*\cite[1]{[\oldcite{#1}]}

\newcommand{\source}[1]{\vspace{-3pt}\caption*{{#1}} }

\newcolumntype{C}[1]{>{\centering}p{#1}}

%\newglossaryentry{Canvas}
%{
%name=Canvas,
%description={oder Leinwand. Bezeichnet eine Fläche eines Fensters in der Computergrafik, %%in die virtuell gezeichnet werden kann}
%}


\newacronym{a}{AA}{AAA}


\makeglossaries
\usepackage[xindy]{imakeidx}
\makeindex


%\addbibresource{bibtex.bib}

\counterwithout{figure}{chapter}
\counterwithout{footnote}{chapter}


\titleformat{\chapter}[display]
   {\normalfont\large\bfseries}
   {\begin{figure}[!htb]\ \centering \includegraphics[width=0.9\linewidth]{images/teaser.png}\ \caption{MRT-Daten mit maskiertem Bereich und ihre Darstellung in mARt als zwei- und dreidimensionale Abbildung. (Eigene Darstellung)}\  \end{figure}\ \chaptertitlename\ \thechapter}{1em}{\large}
   
\titleformat{\section}
   {\normalfont\large\bfseries}
   {\thesection}{1em}{}
   
\titleformat{\subsection}
   {\normalfont\small\bfseries}
   {\thesubsection}{1em}{}


\title{Abschlussarbeit Master Medieninformatik}
\author{Viola Jertschat}

\bibliography{references/references}{}

\begin{document}


\begin{titlepage}
	\centering
	\includegraphics[width=\textwidth]{images/beuthlogo.eps}\par\vspace{1cm}
	{\scshape\LARGE Beuth Hochschule für Technik \par}
	\vspace{1cm}
	{\scshape\Large Abschlussarbeit Master Medieninformatik\par}
	\vspace{1.5cm}
	{\huge\bfseries mARt: Interaktive Darstellung von MRT Daten in AR\par}
	\vspace{2cm}
	{\Large\itshape Viola Jertschat\par}
	\vfill
	betreut von\par
	Prof. Dr.-Ing. Kristian \textsc{Hildebrand}
	
	\vfill
	Zweitgutachter\par
	Prof. Dr. Hartmut \textsc{Schirmacher}

	\vfill

% Bottom of the page
	{\large 01.April 2019\par}
\end{titlepage}

\begin{abstract} 
Der Anwendungsbereich für AR-Apps deckt heutzutage bereits viele Arbeitsfelder ab. Die Technologie erlaubt es, intuitiv und schnell auf Informationen zuzugreifen und Aufgaben so effizienter zu lösen als durch die Verwendung einer Bildschirmanwendung. Dieser Fortschritt sollte vollkommen ausgeschöpft werden, um Arbeitsergebnisse in lebenswichtigen Bereichen, wie der Medizin zu optimieren. 
Eine entscheidende Rolle in diesem Feld spielen bildgebende Verfahren wie MRT-Scans, durch die Ärzte den Körper eines Patienten von innen untersuchen können. 
In dieser Arbeit wird eine AR-Anwendung entwickelt, die die Untersuchung von MRT-Daten ermöglicht, um das Potenzial der Technologie in diesem Szenario zu erforschen. Der Fokus liegt hierbei auf der Visualisierung von Gehirn-Scans, die zur Diagnose und Behandlung von Schlaganfällen dienen. 
In Zusammenarbeit mit einem Neurologen wird dazu eine Anwendung konzipiert und implementiert, die anschließend durch Nutzertests ausgewertet wird.

%\cite{Hadwiger06}

%\citet{Hadwiger06}

%\citep{Hadwiger06}

\end{abstract}

\tableofcontents

\listoffigures

\printglossaries

%\listoftodos

\include{chapters/motivation}

\titleformat{\chapter}[display]
   {\normalfont\large\bfseries}
   {\chaptertitlename\ \thechapter}{1em}{\large}
   
   
\include{chapters/grundlagen}
\include{chapters/anforderung}
\include{chapters/konzept}
\include{chapters/implementierung}
%\include{chapters/ergebnisse}
\include{chapters/evaluation}
\include{chapters/fazit}


\pagebreak

%\printbibliography
\printbibliography[title={Literaturverzeichnis},nottype=misc]
\printbibliography[title={Webseitenverzeichnis},type=misc]

\end{document}
